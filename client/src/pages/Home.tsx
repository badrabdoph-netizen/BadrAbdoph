...
export default function Home() {
  ...
  const { data: dbSections } = trpc.sections.getAll.useQuery(undefined, {
    staleTime: 30_000,
    refetchOnWindowFocus: false,
  });

  const { data: dbTestimonials } = trpc.testimonials.getAll.useQuery(undefined, {
    staleTime: 30_000,
    refetchOnWindowFocus: false,
  });
  ...
  const homeSections = useMemo(() => {
    const defaults = [
      { key: "hero", sortOrder: 1 },
      { key: "about_preview", sortOrder: 2 },
      { key: "portfolio_preview", sortOrder: 3 },
      { key: "services_preview", sortOrder: 4 },
      { key: "testimonials", sortOrder: 5 },
      { key: "cta", sortOrder: 6 },
    ] as const;

    const merged = defaults.map((d) => {
      const db = dbSections?.find((s) => s.key === d.key);
      return {
        key: d.key,
        sortOrder: db?.sortOrder ?? d.sortOrder,
  ...
  const testimonialsToShow = useMemo(() => {
    if (dbTestimonials && dbTestimonials.length > 0) {
      return dbTestimonials
        .filter((t) => t.visible)
        .map((t) => ({ quote: t.quote, name: t.name }));
    }
    return testimonials;
  }, [dbTestimonials]);
          ...
          <header className="relative h-screen w-full overflow-hidden flex items-center justify-center">
            ...
            <div className="relative z-20 container mx-auto px-4 text-center flex flex-col items-center animate-in fade-in zoom-in duration-1000">
              <EditableText
                ...
                className="text-primary text-lg md:text-xl tracking-[0.3em] uppercase mb-4 font-medium"
              />

              <EditableText
                as="h1"
                contentKey="hero_title"
                fallback={photographerInfo.taglineAr}
                category="home"
              ...
              <EditableText
                ...
                fallback={photographerInfo.descriptionAr}
                category="home"
                label="الوصف"
                multiline
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="bg-card p-8 border border-white/5 hover:border-primary/30 transition-all duration-300 group">
                  ...
                  <p className="text-muted-foreground mb-6 leading-relaxed">
                    جلسات تصوير خارجية للعروسين في أماكن مميزة، مع التركيز على
                    الإضاءة الطبيعية والمشاعر العفوية.
                  </p>
                  <ul className="text-sm text-muted-foreground space-y-2 mb-8">
                    <li className="flex items-center">
                      <Star size={14} className="ml-2 text-primary" /> عدد
                      صور غير محدود
                    </li>
                    <li className="flex items-center">
                      <Star size={14} className="ml-2 text-primary" /> شامل 2
                      Reels & TikTok
                    </li>
                    <li className="flex items-center">
                      <Star size={14} className="ml-2 text-primary" /> تعديل
                      احترافي
                    </li>
                  </ul>
                </div>
                ...
                <div className="bg-card p-8 border border-primary/20 relative transform md:-translate-y-4 shadow-2xl shadow-black/50">
                  ...
                  <p className="text-muted-foreground mb-6 leading-relaxed">
                    تغطية شاملة ليوم الزفاف من التحضيرات حتى نهاية الحفل،
                    لتوثيق كل لحظة وكل تفصيل.
                  </p>
                  <ul className="text-sm text-muted-foreground space-y-2 mb-8">
                    <li className="flex items-center">
                      <Star size={14} className="ml-2 text-primary" /> ألبومات
                      فاخرة مطبوعة
                    </li>
                    <li className="flex items-center">
                      <Star size={14} className="ml-2 text-primary" /> تابلوهات
                      خشبية
                    </li>
                    <li className="flex items-center">
                      <Star size={14} className="ml-2 text-primary" /> تغطية
                      فيديو Reels
                    </li>
                  </ul>
                </div>
                ...
                <div className="bg-card p-8 border border-white/5 hover:border-primary/30 transition-all duration-300 group">
                  ...
                  <p className="text-muted-foreground mb-6 leading-relaxed">
                    تجربة تصوير كاملة بمستوى VIP ليوم لا يتكرر، مع فريق عمل
                    متكامل واهتمام بأدق التفاصيل.
                  </p>
                  <ul className="text-sm text-muted-foreground space-y-2 mb-8">
                    <li className="flex items-center">
                      <Star size={14} className="ml-2 text-primary" /> تغطية
                    ...
                    <li className="flex items-center">
                      <Star size={14} className="ml-2 text-primary" /> فيديو
                      برومو سينمائي
                    </li>
                    <li className="flex items-center">
                      <Star size={14} className="ml-2 text-primary" /> هدايا
                      حصرية
                    </li>
                  </ul>
                </div>
            ...
            <div className="container mx-auto px-4 text-center relative z-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                {testimonialsToShow.map((testimonial, index) => (
                  ...
                  >
                    <div className="text-primary text-4xl font-serif mb-4">
                      "
                    </div>
          ...
          <section className="py-32 relative flex items-center justify-center">
            ...
            <div className="relative z-10 text-center px-4">
              <EditableText
                ...
                fallback={pageTexts.home.ctaTitle}
                category="cta"
                label="عنوان قسم الدعوة للتواصل"
              ...
              <EditableText
                ...
                fallback={pageTexts.home.ctaDescription}
                category="cta"
                label="وصف قسم الدعوة للتواصل"
                multiline
